...
def generate_launch_description():
    ...
    # --- Bridge the simulation clock globally ---
    clock_bridge = Node(
    ...
    robots = [
        {'name': 'tb3_leader', 'x': '1.2', 'y': '0.0'}, # Leader
        {'name': 'tb3_follower1', 'x': '0.8', 'y': '0.0'}, # Follower 1
        {'name': 'tb3_follower2', 'x': '0.4', 'y': '0.0'}, # Follower 2
        {'name': 'tb3_follower3', 'x': '0.0', 'y': '0.0'}, # Follower 3
        {'name': 'tb3_T3', 'x': '2.5', 'y': '0.35'}, # T3 from photo
    ]
    ...
    for robot in robots:
        ...
        # Gazebo publishes to /model/<name>/odometry, so remap it to /<name>/odom 
        # so it matches the Python script expectations.
        spawn_cmds.append(
            Node(
                package='ros_gz_bridge',
                executable='parameter_bridge',
                arguments=[
                    ...
                    # Bridge Absolute Pose FROM Gazebo
                    f'/model/{robot["name"]}/pose@geometry_msgs/msg/Pose[gz.msgs.Pose'
                ],
                remappings=[
                    (f'/model/{robot["name"]}/cmd_vel', f'/{robot["name"]}/cmd_vel'),
                    (f'/model/{robot["name"]}/odometry', f'/{robot["name"]}/odom'),
                    (f'/model/{robot["name"]}/pose', f'/{robot["name"]}/pose')
                ],
                output='screen'
            )
        )
    ...
    return LaunchDescription([
        ...
        set_model_path,
        gz_sim,
        clock_bridge, # Added the clock bridge here
    ] + spawn_cmds)